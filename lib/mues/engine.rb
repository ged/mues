...
# == Rcsid
# 
# $Id: engine.rb,v 1.46 2004/02/29 04:36:14 deveiant Exp $
# 
# == Authors
# 
...
class Engine < MUES::Object ; implements MUES::Debuggable
	...
	Rcsid				= %q$Id: engine.rb,v 1.46 2004/02/29 04:36:14 deveiant Exp $
	...
	def handleListenerConnect( sock, event, listener )
		...
		when :read
			...
			case listener.loginQuestionnaire
			when MUES::Questionnaire
				qnaire = listener.loginQuestionnaire.dup
			when String
				name = listener.loginQuestionnaire 
				qnaire = MUES::Questionnaire::load( name )
			end

			qnaire.data[:filter] = ofilter
			filters << listener.loginQuestionnaire

			self.dispatchEvents( ListenerConnectEvent::new(listener, *filters) )
