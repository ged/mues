...
class EventQueue < Object ; implements MUES::Debuggable
	...
	def shutWorkersDown
		...
		@workers.list.each {|worker|
			self.log.warning "Forcefully killing worker thread #{worker.desc}"
			killWorkerThread( worker )
		}
	end
	...
	def killWorkerThread( workerThread )
		raise ArgumentError,
			"Cannot kill the current thread" if
			workerThread == Thread.current
		raise ArgumentError,
			"Argument must be a worker thread" unless
			workerThread.is_a?( WorkerThread )
