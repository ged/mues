...
module TelnetConstants
	...
	CMD = CMDCODE.invert
	CMDCODE.each {|cmd,code| TelnetConstants.const_set(cmd, code) }
	...
	OPT = OPTCODE.invert
	OPTCODE.each {|opt,code| TelnetConstants.const_set("TELOPT_#{opt}", code) }

	# Telnet suboption qualifiers
	TELQUALCODE = {
		'IS'	=> 0.chr,		# "Here is"
		'SEND'	=> 1.chr,		# "Please send"
		'INFO'	=> 2.chr,		# "Info about"
		'REPLY'	=> 2.chr,		# "Reply to"
		'NAME'	=> 3.chr,		# "Name of"
	}
	TELQUAL = TELQUALCODE.invert
	TELQUALCODE.each {|qual,code| TelnetConstants.const_set("TELQUAL_#{qual}", code) }
