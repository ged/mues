...
user = Readline.readline( "login: " )
pass = prompt_for_password( "password: " )
char = Readline.readline( "character: " )
...
player_exch = broker.exchange( "#{user}:#{char}", :type => :topic, :durable => true, :auto_delete => true )
player_queue = broker.queue( "#{user}:#{char}", :exclusive => true )
player_queue.bind( player_exch )

# Publish a request for connection to the specified character
connect_exchange = broker.exchange( :login, :passive => true )
connect_exchange.publish( "#{user}:#{char}", :key => :character_name, :mandatory => true )
...
if msg == :no_return
	...
	t1 = Thread.new do
		...
		player_queue.subscribe( :header => true, :consumer_tag => "#{char}_events", 
